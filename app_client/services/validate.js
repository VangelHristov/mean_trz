(function validationFactoryModule() {
	'use strict';

	angular
		.module('app')
		.factory('validate', function validate() {
			...
			const isValidEgn = function (egn) {
				...
				let year = yearFlag + Number(egn.substr(0, 2));
				dateBorn = new Date(year, monthBorn, dayBorn, 0, 0, 0);
			...
			const isValidIban = function isValidIban(input) {
				let codeLengths = {
					AD: 24,
					AE: 23,
					AT: 20,
			...
			const isValidBulstat = function (bulstat) {
				const
				...
				function getNinthDigit(bulstat) {
					...
					if (ninthDigit % 11 === 10) {
						ninthDigit = loop(
							nineDigitsSecondCtrlNumbers,
							bulstat,
							0
						);
					}

					return ninthDigit % 11 === 10
						? 0
						: ninthDigit % 11;
				...
				function getThirteenthDigit(bulstat) {
					...
					let thirteenthDigit = loop(
						thirteenDigitsFirstCtrlNumbers,
						bulstat,
						9
					);
					if (thirteenthDigit % 11 === 10) {
						thirteenthDigit = loop(
							thirteenDigitsSecondCtrlNumbers,
							bulstat,
							9
						);
					}
				...
				switch (bulstat.trim().length) {
					case 9 :
						return getNinthDigit(bulstat) === Number(bulstat[8]);
					case 10:
						return isValidEgn(bulstat);
					case 13:
						return getThirteenthDigit(bulstat) === Number(bulstat[12]);
					default:
						return false;
				}
			};
